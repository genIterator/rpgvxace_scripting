...
# ** Rest of the script
#==============================================================================
    Stype_Regex = /<skill[-_ ]type:\s*(.*)\s*>/i
...
module RPG
  class Skill < UsableItem
    ...
    def load_notetags_multiple_stypes
      @stype_ids = [@stype_id]
      res = self.note.match(TH::Skill_Type_Groups::Stype_Regex)
      if res
        res[1].scan(/\d+/).each {|id| @stype_ids << id.to_i}
      end
    end
  end
end
...
class Window_SkillList < Window_Selectable
  ...
  alias :th_Skill_Type_Groups_list_include? :include?
  def include?(item)
    item && (TH::Skill_Type_Groups.include?(@stype_id, item.stype_ids) || item.stype_ids.include?(@stype_id)) || th_Skill_Type_Groups_list_include?(item)
  end
end