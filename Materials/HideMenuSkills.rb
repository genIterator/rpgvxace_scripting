...
module YEA
  ...
  module SKILL
    ...
    HIDE_WHENNOT_EQUIP = /<(?:HIDE_WHENNOT_EQUIP|hide when not equip):[ ](.*)>/i
    HIDE_IN_BATTLE_WHENNOT_EQUIP = /<(?:HIDE_IN_BATTLE_WHENNOT_EQUIP|hide in battle when not equip):[ ](.*)>/i
...
class RPG::Skill < RPG::UsableItem
  ...
  attr_accessor :hide_whennot_equip
  attr_accessor :skillType
  attr_accessor :hide_in_battle_whennot_equip
  ...
  def load_notetags_hms
    ...
    self.note.split(/[\r\n]+/).each { |line|
      ...
      when YEA::REGEXP::SKILL::HIDE_EVAL_OFF
        @hide_eval_on = false
      when YEA::REGEXP::SKILL::HIDE_WHENNOT_EQUIP
        @hide_whennot_equip = true
        @skillType = $1
      when YEA::REGEXP::SKILL::HIDE_IN_BATTLE_WHENNOT_EQUIP
        @hide_in_battle_whennot_equip = true
        @skillType = $1
      else
...
class Window_SkillList < Window_Selectable
  ...
  def include?(item)
    ...
    ######################
    #added stuff
    unless @actor.nil? # if actor != nil
      if (item.hide_whennot_equip || item.hide_in_battle_whennot_equip) # var hide_whennot_equip or hide_in_battle_whennot_equip is set (logisch?)
        return hide_untilEquipped(item)
      end
    end
    ######################
  ...
  #--------------------------------------------------------------------------
  # new method: hide_untilEquipped(item)
  #--------------------------------------------------------------------------  
  def hide_untilEquipped(item)
    weapMainHand = ""
    weapOffHand = ""
    tempA = false
    tempB = false
    ...
    if !(@actor.equips[0].nil?)
      weapMainHand = $data_system.weapon_types[@actor.equips[0].wtype_id];
      if (weapMainHand.eql?(item.skillType))
        tempA = true;
      end
      @actor.equips[0].features.each do |obj|
        if (obj.code == 54) #Seal Shield
          twoHandWeapon = true;
        end
      end
    end   
    if (!(@actor.equips[1].nil?) && (twoHandWeapon == false) && (@actor.equips[1].etype_id == 0)) #Weap1 = twohand?, or dual wield?
      weapOffHand = $data_system.weapon_types[@actor.equips[1].wtype_id];
      if (weapOffHand.eql?(item.skillType))
        tempB = true
     end
    end
    if (item.hide_whennot_equip)
      if (tempA || tempB)
        return true
      end
    end
    if (item.hide_in_battle_whennot_equip && $game_party.in_battle)
      if (tempA || tempB) 
              return true
      end
    end
